...
void main() async {
  ...
  group('ApiProvider', () {
    ...
    test('get movie detail by id ', () async {
      MockDio dio = MockDio();
      ApiProvider apiProvider = ApiProvider(dioApi: dio);
      const String movieId = "653346";

      // Stub the response of dio.get with headers
      when(() => dio.get("https://api.themoviedb.org/3/movie/$movieId/videos"))
          .thenAnswer(
        (_) async => Response(
          statusCode: 200,
          requestOptions: RequestOptions(
            path: "https://api.themoviedb.org/3/movie/$movieId/videos",
      ...
      final response = await apiProvider.fetchMovieDetailById(movieId);

      // Verify that the response data matches the expected data
      // Verify that the response data matches the expected data
      // For example, you can check the length of the results list
      expect(response.title, "Kingdom of the Planet of the Apes");
